...
class EventData: ObservableObject {
    ...
    func sortedEvents(period: Period) -> Binding<[Event]> {
        Binding<[Event]> {
            self.events.filter {
                switch period {
                case .nextSevenDays: return $0.isWithinSevenDays
                case .nextThirtyDays: return $0.isWithinSevenDaysToThirtyDays
                case .future: return $0.isDistant
                case .past: return $0.isPast
                }
            }
        } set: { events in
            for event in events {
